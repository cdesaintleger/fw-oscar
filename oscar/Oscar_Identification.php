...
class Oscar_Identification{
	 ...
	 */
	private static $_ident_table_name	=	null;
	
	/*
	 * Nom du champ pour les loggin
	 */
	private static $_login_field_name	=	null;
	
	/*
	 * Nom du champ pour les mots de passe
	 */
	private static $_password_field_name	=	null;
	
	/*
	 * Nom du champ pour l'etat du compte 1 actif 0 inactif
	 */
	private static $_status_field_name	=	null;
	...
	/*
	 * Instance pour se connecter à la BDD
	 */
	private static $_Instance_Ddb	=	null;
	 ...
	 * 
	 */
	private function __construct( $ident_table_name=null, $login_field_name=null, $password_field_name=null, $typeauth="session"){
		
		self::$_ident_table_name		=	$ident_table_name;
		self::$_login_field_name		=	$login_field_name;
		self::$_password_field_name		=	$password_field_name;
		//self::$_status_field_name		=	$status_field_name;
	...
	private function _session_client_is_valide(){
		...
		$sql	=	'SELECT * FROM '.self::$_oscar_temp_table_name.' 
				WHERE 
			...
			if($statement = self::$_Instance_Ddb->prepare($sql)){
				
				switch(self::$_type_auth){
							
					case "session":
						
						$Tvalues	=	array($_SESSION['Oscar_i']['oscar_key'],$_SESSION['Oscar_i']['oscar_log']);
						
					break;
					
					case "cookie":
						
						$Tvalues	=	array($_COOKIE['Oscar_i']['oscar_key'],$_COOKIE['Oscar_i']['oscar_log']);
						
					break;
				}
				
				//Execute la requete
				if($statement->execute($Tvalues)){
					...
					if(count($red)>0){
						 ...
						 */
                                                ...
                                                if( self::$_test_ip ){
                                                    ...
                                                    if($red[0][self::$_oscar_ip_user] != $sha1ip){
                                                        $test=FALSE;
                                                        //Log de l'a tentative de la deconnexion
                                                        if(self::$_active_log){
                                                ...
                                                if( self::$_test_nav ){
                                                    if($red[0][self::$_oscar_nav_user] != $sha1navigateur){
                                                        $test=FALSE;
                                                        //Log de l'a tentative de la deconnexion
                                                        if(self::$_active_log){
						 ...
						 */
						self::$_private_data	=	unserialize($red[0][self::$_oscar_data]);
	...
	public function identify($login, $password){
		...
		//Si nous avons un acces , à la base de données
		if(!empty(self::$_Instance_Ddb)){
			
			$sql	=	'SELECT * , COUNT('.self::$_login_field_name.') AS nb_result FROM '.self::$_ident_table_name.' 
				WHERE 
					'.self::$_login_field_name.' = ?
				AND 
					'.self::$_password_field_name.' = ? 
				GROUP BY '.self::$_login_field_name.' 
				LIMIT 1';
			
						
			if($statement = self::$_Instance_Ddb->prepare($sql)){
				
				$TabValues	=	array($login,$this->_cryptage($password));
				
				//Execute la requete
				if($statement->execute($TabValues)){
					//récupére le resultat
					$red = $statement->fetchAll(PDO::FETCH_ASSOC);
					
					//test le resultat
					if($red[0]['nb_result'] > 0){
	...
	private function _create_temp_record($infos	=	array()){
		...
		if(!empty($sha1login)){
			...
			//Supprime les anciens s'ils exitent
			$sql	=	'DELETE FROM '.self::$_oscar_temp_table_name. '
				WHERE '.self::$_oscar_user_login.'	=	?';
			
			//Prépare la requete
			if($statement = self::$_Instance_Ddb->prepare($sql)){
				//Execute la requete
				$statement->execute(array($sha1login));
			}
			...
			if($statement = self::$_Instance_Ddb->prepare($sql)){
				...
				$statement->bindParam(':login', $sha1login);
				$statement->bindParam(':oscar_key', $sha1oscar_key);
				$statement->bindParam(':ip', $sha1ip);
				$statement->bindParam(':navigateur', $sha1navigateur);
				$statement->bindParam(':private_data',$_serialize_private_data);
				$statement->bindParam(':t_creation', $t_creation);
				$statement->bindParam(':t_expiration', $t_expiration);
				
				//Execute la requete
				if($statement->execute()){
					
					//Success de l'operation retourne l'id
					$retour	=	self::$_Instance_Ddb->lastInsertId();
